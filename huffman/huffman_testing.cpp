#pragma warning(disable : 4996)
#include<algorithm>
#include <cassert>
#include <cstdlib>
#include <vector>
#include <utility>
#include "gtest/gtest.h"
#include "huffman_interface.h"
#include <sstream>

namespace {
	std::string encode_decode_str(std::string str) {
		std::ofstream test_writer("encode.txt", std::ios::binary);
		test_writer << str;
		test_writer.close();

		huf_interface::encode("encode.txt", "tmp");
		huf_interface::decode("tmp", "decode.txt");

		std::ifstream out_test("decode.txt", std::ios::binary);
		std::string check((std::istreambuf_iterator<char>(out_test)), std::istreambuf_iterator<char>());
		out_test.close();
		return check;
	}

	bool random_test(size_t size) {
		std::ofstream in_test("encode.txt", std::ios::binary);
		char* in = new char[size];
		for (size_t i = 0; i < size; ++i) {
			in[i] = (char)rand();
		}
		in_test.write(in, size);
		in_test.close();
		huf_interface::encode("encode.txt", "tmp");
		huf_interface::decode("tmp", "decode.txt");
		char *buf1 = new char[BUF_SIZE];
		char *buf2 = new char[BUF_SIZE];

		std::ifstream f1("encode.txt");
		std::ifstream f3("decode.txt");
		bool f = true;
		while (true) {
			f1.read(buf1, BUF_SIZE);
			int gc1 = (int)f1.gcount();
			f3.read(buf2, BUF_SIZE);
			int gc3 = (int)f3.gcount();
			if (gc1 != gc3)
				f = false;
			for (int i = 0; i < gc1; ++i)
				if (buf1[i] != buf2[i])
					f = false;
			if (f1.eof())
				break;
		}
		delete[] buf1;
		delete[] buf2;
		f1.close();
		f3.close();
		return f;
	}
	
	bool equal(std::string file1, std::string file2)
	{
		encoder e;
		e.encode(file1, "tmp");
		decoder d;
		d.decode("tmp", file2);
		char *buf1 = new char[BUF_SIZE];
		char *buf2 = new char[BUF_SIZE];

		std::fstream f1(file1);
		std::fstream f3(file2);
		bool f = true;
		while (true) {
			f1.read(buf1, BUF_SIZE);
			int gc1 = (int)f1.gcount();
			f3.read(buf2, BUF_SIZE);
			int gc3 = (int)f3.gcount();
			if (gc1 != gc3)
				f = false;
			for (int i = 0; i < gc1; ++i)
				if (buf1[i] != buf2[i])
					f = false;
			if (f1.eof())
				break;
		}
		delete[] buf1;
		delete[] buf2;
		f1.close();
		f3.close();
		return f;
	}
}

TEST(correctness, hello_world)
{
    std::string str = "hello world";
    std::string check = encode_decode_str(str);
    EXPECT_EQ(str, check);
}

TEST(correctness, empty)
{
    std::string str = "";
    std::string check = encode_decode_str(str);
    EXPECT_EQ(str, check);
}

TEST(correctness, one_letter)
{
	std::string str = "a";
	std::string check = encode_decode_str(str);
	EXPECT_EQ(str, check);

	str = "aaaaaaa";
	check = encode_decode_str(str);
	EXPECT_EQ(str, check);
}

namespace {

}

TEST(correctness, big_random)
{
    for (size_t i = 0; i < 10; ++i) {
        EXPECT_TRUE(random_test(10));
    }
}

TEST(correctness, buffer_random) {
	EXPECT_TRUE(random_test(BUF_SIZE * 8 + BUF_SIZE/3));
}

TEST(correctness, jpg) {
	EXPECT_TRUE(equal("test1.jpg", "out1.jpg"));
	EXPECT_TRUE(equal("test2.JPG", "out2.JPG"));
}